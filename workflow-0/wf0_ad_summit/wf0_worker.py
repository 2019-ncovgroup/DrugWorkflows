...
class MyWorker(rp.task_overlay.Worker):
    ...
    def pre_exec(self):
        ...
        try:
            ...
            receptor_glob      = 'input_dir/receptors.v7/%s*'    % workload.receptor
            smiles_file        = 'input_dir/smiles/%s.csv'       % workload.smiles
            output             = './out.%s.sdf'                  % self._uid


            out, err, ret = ru.sh_callout("mkdir -p %s" % workload.cache)
            assert(ret), [out, err, ret]

            out, err, ret = ru.sh_callout("cp %s %s" % (receptor_glob, cache), shell=True)
            assert(ret), [out, err, ret]

            out, err, ret = ru.sh_callout("cp %s %s" % (workload.can,  cache), shell=True)
            assert(ret), [out, err, ret]
    ...
    def post_exec(self):
        ...
        try:
            ...
            out, err, ret = ru.sh_callout("cp -r %s/results/ ." % cache)
            assert(ret), [out, err, ret]
