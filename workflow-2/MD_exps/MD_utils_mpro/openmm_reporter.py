...
class ContactMapReporter_sel(object):
    def __init__(self, file, reportInterval, res_dict):
        self._file = h5py.File(file, 'w', libver='latest')
        self._file.swmr_mode = True
        self._res1 = res_dict['x'] 
        self._res2 = res_dict['y'] 
        self._out = self._file.create_dataset('contact_maps', shape=(0,0,0), maxshape=(None, None,None))
        self._reportInterval = reportInterval
    ...
    def report(self, simulation, state):
        ...
        distance_matrix = distances.distance_array(
                positions_ca[self._res1], positions_ca[self._res2]) 
        contact_map = (distance_matrix < 8.0) * 1.0 
