...
def generate_training_pipeline(cfg):
    ...
    def generate_MD_stage(num_MD=1):
        ...
        for i in range(num_MD):
            ...
            t1.pre_exec = ['. /etc/profile.d/conda.sh', 'module load cuda/9.1.85']
    ...
    def generate_aggregating_stage():
        ...
        cnt_constraint = min(cfg['node_counts'] * cfg['gpu_per_node'], cfg['md_counts'] * max(1,
            CUR_STAGE) // 2, cfg['cpu_per_node'] - 4)


        t2.executable = ['%s/bin/python' % (cfg['conda_pytorch'])]  # MD_to_CVAE.py
        pnodes = 1
        cmd_cat    = 'cat /dev/null'
        cmd_jsrun  = 'jsrun -n %s -r 1 -a 4 -c 10 -d packed' % cfg['node_counts']
        cmd_vae = '%s/bin/python' % (cfg['conda_pytorch'])
        t2.executable = ['%s; %s %s' % (cmd_cat, cmd_jsrun, cmd_vae)]
        t2.arguments = [
    ...
    def generate_interfacing_stage():
        ...
        t4.pre_exec = ['. /etc/profile.d/conda.sh', 'conda activate %s' % cfg['conda_pytorch']]
        ...
        cmd_jsrun = 'jsrun -n %s -a %s -g %s -r 1 -c %s' % (1,1, cfg['gpu_per_node'], cfg['cpu_per_node'])
        ...
        t4.cpu_reqs = {'processes'          : cfg['gpu_per_node'] * cfg['node_counts'],
                       'process_type'       : 'MPI',
...
if __name__ == '__main__':
    ...
    reporter.title('DeepDriveMD - exp1')
