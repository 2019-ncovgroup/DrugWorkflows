...
#
# [1] Lassen: https://hpc.llnl.gov/software/banks-running-jobs
# [2] Workflow: https://docs.google.com/document/d/1XFgg4rlh7Y2nckH0fkiZTxfauadZn_zSn3sh51kNyKE/
#
...
TASK_PRE_EXEC_MODULES = [
    ...
    'source %s/amber.sh' % os.environ.get('AMBERHOME')]
...
def generate_training_pipeline():
    ...
    def generate_MD_stage(num_MD=1):
        ...
        for i in range(num_MD):
            ...
            t1.executable = ['python']  # run_openmm.py
            ...
            if initial_MD or i >= len(outlier_list):
                ...
                sim_path = os.path.join(md_path,
                                        os.path.basename(pdb_file)[:-11])
                ...
                sim_path = os.path.join(md_path,
                                        os.path.basename(check_point)[:-4])
...
if __name__ == '__main__':
    ...
    with open('creds_lassen.json') as f:
        creds = json.load(f)
    os.environ['RADICAL_PILOT_DBURL'] = creds['mongodb']['url']
    # Create Application Manager
    appman = AppManager(**creds['rabbitmq'])
