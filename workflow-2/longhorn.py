...
def generate_training_pipeline():
    ...
    def generate_MD_stage(num_MD=1): 
        ...
        for i in range(num_MD):
            ...
            t1.pre_exec = ['module load cuda/10.1']
            t1.pre_exec += [ 'export OMPI_LD_PRELOAD_POSTPEND_DISTRO=/opt/ibm/spectrum_mpi/lib/libpami_cudahook.so',
            ...
            t1.pre_exec += ['. /scratch/apps/conda/4.8.3/etc/profile.d/conda.sh']
            t1.pre_exec += ['source /scratch/apps/conda/4.8.3/etc/profile.d/conda.sh']
    ...
    def generate_aggregating_stage(): 
        ...
        t2.pre_exec = [] 
        t2.pre_exec = ['module load conda/4.8.3', 'module load cuda/10.1']
        t2.pre_exec += [ 'export OMPI_LD_PRELOAD_POSTPEND_DISTRO=/opt/ibm/spectrum_mpi/lib/libpami_cudahook.so',
            'export LD_PRELOAD="/opt/ibm/spectrum_mpi/lib/pami_noib/libpami.so $OMPI_LD_PRELOAD_POSTPEND_DISTRO $LD_PRELOAD"',
            'export TACC_SPECTRUM_ENV=`/usr/local/bin/build_env.pl | sed -e\'s/\(\S\S*\)=\S\S* / -x \\1/g\'`']

        t2.pre_exec += ['. /scratch/apps/conda/4.8.3/etc/profile.d/conda.sh', 'conda init bash']
    ...
    def generate_ML_stage(num_ML=1): 
        ...
        for i in range(num_ML): 
            ...
            t3.pre_exec = ['module load cuda/10.1']
            t3.pre_exec += [ 'export OMPI_LD_PRELOAD_POSTPEND_DISTRO=/opt/ibm/spectrum_mpi/lib/libpami_cudahook.so',
            'export LD_PRELOAD="/opt/ibm/spectrum_mpi/lib/pami_noib/libpami.so $OMPI_LD_PRELOAD_POSTPEND_DISTRO $LD_PRELOAD"',
            'export TACC_SPECTRUM_ENV=`/usr/local/bin/build_env.pl | sed -e\'s/\(\S\S*\)=\S\S* / -x \\1/g\'`'] 
            t3.pre_exec += ['. /scratch/apps/conda/4.8.3/etc/profile.d/conda.sh']
            t3.pre_exec += ['source /scratch/apps/conda/4.8.3/etc/profile.d/conda.sh']
    ...
    def generate_interfacing_stage(): 
        ...
        t4 = Task() 
        t4.pre_exec = [] 
        t4.pre_exec = ['module load conda/4.8.3', 'module load cuda/10.1']
        t4.pre_exec += [ 'export OMPI_LD_PRELOAD_POSTPEND_DISTRO=/opt/ibm/spectrum_mpi/lib/libpami_cudahook.so',
            'export LD_PRELOAD="/opt/ibm/spectrum_mpi/lib/pami_noib/libpami.so $OMPI_LD_PRELOAD_POSTPEND_DISTRO $LD_PRELOAD"',
            'export TACC_SPECTRUM_ENV=`/usr/local/bin/build_env.pl | sed -e\'s/\(\S\S*\)=\S\S* / -x \\1/g\'`'] 

        t4.pre_exec += ['. /scratch/apps/conda/4.8.3/etc/profile.d/conda.sh']
        t4.pre_exec += ['source /scratch/apps/conda/4.8.3/etc/profile.d/conda.sh']
